...
class ResBlk1(nn.Module): #设计一个残差模块
    ...
    def __init__(self, ch_in=8, ch_out=16, stride=2):
        ...
        nn.Conv1d(8,16,kernel_size=3,stride=3,padding=1),
        nn.BatchNorm1d(16),
        nn.ReLU(),
        ...
        if ch_out != ch_in:
            self.extra = nn.Sequential(
                nn.Conv1d(ch_in, ch_out, kernel_size=1, stride=6,padding=0),
                nn.BatchNorm1d(ch_out),  
            )
        # tmp1 = torch.randn(40, 8, 134)    #torch.Size([40, 8, 49, 49]) 第一层以后的shape
        # out2_1=self.conv2_unit1(tmp1) #torch.Size([40, 32, 23])
        # print(out2_1.shape)#
        # out2_2=self.extra(tmp1)#torch.Size([40, 32, 23])
        # print(out2_2.shape)
    def forward(self, x):
...
class NIR2(nn.Module):
    def __init__(self):
        ...
        self.conv1_unit=nn.Sequential(
            nn.Conv1d(1,8,kernel_size=3,stride=3,padding=0),
            nn.BatchNorm1d(8),
            nn.ReLU(),#激活函数
        ...
        out_n=self.conv1_unit(tmp)#torch.Size([40, 8, 134])
